...
## Curio Onboarding Decisions

- Keep the app plain and lightweight: warm white background, flat controls, no room scene, and no large illustrated background.
- Fill the phone screen vertically and keep Curio comfortably large, centered, and fully inside the Rive stage; do not scale the Rive canvas beyond its container.
- Use the supplied `public/assets/curio/curio-getting-started.riv` file as the character source.
- The Rive state machine is `State Machine 1`; its `ff` trigger moves forward and `bw` moves backward.
- The initial Rive state is Curio's idle welcome. The first `ff` trigger enters the grown-up setup animation.
...
- On Desktop, show Getting Started as a balanced two-column layout with Curio on the left and the onboarding content on the right, centered through the usable height.
